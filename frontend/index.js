...
// Weiterleitung zur passenden HTML-Seite basierend auf der ersten gültigen Rolle
function redirectToRolePage(roles) {
  const validRoles = ['admin', 'provider', 'user'];
  const matchedRole = validRoles.find(role => roles.includes(role));

  if (matchedRole) {
    window.location.href = `/src/pages/${matchedRole}/${matchedRole}.html`;
  } else {
